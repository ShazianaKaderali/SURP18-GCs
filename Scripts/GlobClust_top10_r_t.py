...
GlobClust_Log = pd.read_csv('~/GlobClust_l-b-r_t.csv')
#GlobClust_Log = GlobClust_Log.set_index("ID", drop = True) #--> List by ID for index
GlobClust_Log_short = GlobClust_Log.loc[0:10,"ID":"r_t "]
...
GlobClust_Log_r_t = GlobClust_Log_short.loc[:,"r_t "]
...
for i in range(0,10): #Should be better
    ...
    abs(b-(%f))<%f""" %(GlobClust_Log_l[i],GlobClust_Log_r_t[i]/60*2,GlobClust_Log_b[i],GlobClust_Log_r_t[i]/60*2)

    out=query.query(globquery,local=True,timeit=True)

    df_t=pd.DataFrame(np.array(out))
    df_t.to_csv('out_'+ GlobClust_Log_ID[i] +'_full_4-r_t.csv', index=True, header=True)
    ...
    plt.title('Window (2*r_t):' +'b (deg) vs l (deg) of ' + GlobClust_Log_ID[i], fontsize=15) #GlobClust_Log_ID[i]
    cb=plt.colorbar()
    ...
    plt.show()
    fig.savefig('GC_'+ GlobClust_Log_ID[i] +'_full_pmra_2-r_t.pdf')
    ...
    #plt.gca().set_xlim([307.25,307.45])
    plt.show()
    fig.savefig('GC_'+ GlobClust_Log_ID[i] +'_full_pmdec_2-r_t.pdf')
