...
for i in range(0,10): #Should be better
    ...
    abs(b-(%f))<%f """ %(GlobClust_Log_l[i],GlobClust_Log_r_h[i]*10,GlobClust_Log_b[i],GlobClust_Log_r_h[i]*10)

    out=query.query(globquery,local=True,timeit=True)

    df_t=pd.DataFrame(np.array(out))
    df_t.to_csv('out-'+ GlobClust_Log_ID[i] +'full_10-r_h.csv', index=True, header=True)
    ...
    plt.title('Large Window (10*r_h):' +'b (deg) vs l (deg) of ' + GlobClust_Log_ID[i], fontsize=15) #GlobClust_Log_ID[i]
    cb=plt.colorbar()
    ...
    plt.show()
    fig.savefig('GC_'+ GlobClust_Log_ID[i] +'_full_pmra_10-r_h.pdf')
    ...
    #plt.gca().set_xlim([307.25,307.45])
    plt.show()
    fig.savefig('GC_'+ GlobClust_Log_ID[i] +'_full_pmdec_10-r_h.pdf')
